...
def ask_real_ai(question, context):
    """Calls the real Claude API using data already computed for this stock. Returns None if unavailable."""
    client = get_ai_client()
    if client is None:
        return None

    fundamentals_text = ", ".join(
        f"{k}: {format_fundamental_value(k, v)}" for k, v in context.get("fundamentals", {}).items()
    ) or "not available"

    news_text = "not available"
    news_df = context.get("news_df")
    if news_df is not None and not news_df.empty:
        news_text = "; ".join(news_df["Title"].head(5).tolist())

    data_summary = f"""
Ticker: {context['ticker']}
Risk score: {context['risk_score']}/100
Volume anomalies detected: {len(context['volume_spikes'])}
Price anomalies detected: {len(context['price_anomalies'])}
Pump-and-dump style flags: {len(context['pump_dump_df'])}
Volume-price divergence flags: {len(context['divergence_df'])}
Total return over selected period: {context['total_return']:.2f}%
Data quality score: {context['dq_score']}/100
Model confidence score: {context['confidence_score']}/100
Fundamentals: {fundamentals_text}
Recent news headlines: {news_text}
"""

    system_prompt = (
        "You are a financial data assistant embedded in a student research app. "
        "Answer the user's question using ONLY the data provided below. "
        "Do not predict future prices. Do not give buy/sell investment advice - "
        "if asked, explain the relevant facts from the data and remind the user this is not financial advice. "
        "Keep answers concise (under 150 words) and clearly grounded in the data given."
    )
    ...
    try:
        message = client.messages.create(
            ...
            messages=[{
                "role": "user",
                "content": f"Data for {context['ticker']}:\n{data_summary}\n\nQuestion: {question}"
            }]
        )
        return message.content[0].text
    except Exception as e:
        return f"AI request failed ({e}). Showing rule-based answer instead below."
    ...
    if analyze_btn:
        ...
        if data is not None and not data.empty:
            ...
            with tab4:
                ...
                ai_client_check = get_ai_client()
                if ai_client_check is not None:
                    st.caption(
                        "Real AI mode is active (Claude API connected). Ask in your own words about this "
                        "stock's risk, volume, price moves, news, fundamentals, or returns."
                    )
                else:
                    st.caption(
                        "Free rule-based mode (no AI API key configured). Ask about risk, volume, price moves, "
