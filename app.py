...
use_max_history = st.sidebar.checkbox(
    "Use maximum available history (instead of years slider)",
    value=False,
    help="Fetches all historical data available from the free data source, which may go back to the "
         "company's listing date. Very few Indian stocks have digital data before the mid-1990s."
)
years = st.sidebar.slider("Years of history", 1, 10, 5, disabled=use_max_history)
...
def fetch_data(ticker, years, use_max=False):
    if use_max:
...
def ask_real_ai(question, context):
    """Calls the real Gemini API (free tier) using data already computed for this stock. Returns None if unavailable."""
    api_key = get_gemini_api_key()
    if not api_key:
        return None
    answer, success = ask_gemini(question, context, api_key)
    return answer


def build_context_summary(context):
    """Builds a compact text summary of the already-computed analysis, to ground the AI's answer."""
    ...
    if data is not None and not data.empty:
        latest_price_line = f"Latest available closing price: Rs {float(data['Close'].iloc[-1]):.2f}"
        latest_date_line = f"Data as of (last available trading date, delayed/not real-time): {data.index[-1].strftime('%Y-%m-%d')}"
        high_low_line = f"Period high: Rs {float(data['Close'].max()):.2f}, Period low: Rs {float(data['Close'].min()):.2f}"
    lines = [
        f"Today's actual date (for reference): {datetime.now().strftime('%Y-%m-%d')}",
        f"Company/ticker: {ticker}",
        latest_price_line,
        latest_date_line,
        high_low_line,
        f"Risk score: {context['risk_score']}/100 ({label})",
        f"Total return over selected period: {context['total_return']:.2f}%",
        f"Volume anomalies detected: {len(context['volume_spikes'])}",
        f"Price anomalies detected: {len(context['price_anomalies'])}",
        f"Pump-and-dump style pattern flags: {len(context['pump_dump_df'])}",
        f"Volume-price divergence flags: {len(context['divergence_df'])}",
        f"Data quality score: {context['dq_score']}/100",
        f"Model confidence score: {context['confidence_score']}/100",
    ]
    lines = [l for l in lines if l]
    fundamentals = context.get("fundamentals", {})
    if fundamentals:
        for k, v in fundamentals.items():
            lines.append(f"{k}: {format_fundamental_value(k, v)}")
    news_df = context.get("news_df")
    if news_df is not None and not news_df.empty:
        lines.append("Recent news headlines:")
        for _, row in news_df.head(5).iterrows():
            lines.append(f"- {row['Title']} ({row.get('Publisher', 'Unknown')})")

    portfolio = context.get("portfolio")
    if portfolio:
        lines.append("User's virtual paper trading position for this stock:")
        lines.append(f"Shares held: {portfolio.get('shares_held', 0)}")
        lines.append(f"Average buy price: Rs {portfolio.get('avg_buy_price', 0):.2f}")
        lines.append(f"Unrealized P&L: Rs {portfolio.get('unrealized_pnl', 0):,.2f}")
        lines.append(f"Virtual cash remaining: Rs {portfolio.get('cash', 0):,.2f}")
...
def ask_gemini(question, context, api_key):
    """
    Calls the Google Gemini API (free tier, no credit card) grounded only in the
    already-computed analysis data for this stock. Falls back gracefully on any error.
    """
    context_summary = build_context_summary(context)
    prompt = (
        "You are a research assistant inside a student-built stock analysis tool. "
        "Answer the user's question using ONLY the data below. Do not invent numbers. "
        "The price and date given below are the latest available - the data source is delayed, not real-time, "
        "so be honest about that if asked about 'live' or 'current' price. "
        "This tool only has historical data going back a limited number of years (up to 10), not decades - "
        "if asked about very old dates (e.g. the 1990s) that are not in the data below, say so honestly rather "
        "than guessing. "
        "Do not give investment advice or tell the user to buy or sell - explain the facts "
        "and let them decide. Keep the answer concise (under 150 words) and in plain language.\n\n"
        f"DATA:\n{context_summary}\n\n"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return f"AI service returned an error (status {response.status_code}). Showing rule-based answer instead.", False
    except Exception as e:
        return f"Could not reach the AI service ({e}). Showing rule-based answer instead.", False
...
def answer_stock_question(question, context):
    ...
    if any(w in q for w in ["risk", "score", "safe", "dangerous", "danger", "high", "low", "why", "worried", "worry", "concern"]):
        label, _ = risk_label(context["risk_score"])
        answers.append(
    ...
    if any(w in q for w in ["return", "trend", "performance", "gain", "loss", "growth", "grow", "growing", "up", "down", "increase", "decrease"]):
    ...
    if any(w in q for w in ["p&l", "pnl", "profit", "loss", "my position", "my portfolio", "my holding", "how much did i"]):
        portfolio = context.get("portfolio")
        if portfolio and portfolio.get("shares_held", 0) > 0:
            answers.append(
                f"**Your paper trading position:** You hold {portfolio['shares_held']} virtual shares of {ticker} "
                f"at an average buy price of Rs {portfolio['avg_buy_price']:.2f}. Your unrealized P&L is "
                f"Rs {portfolio['unrealized_pnl']:,.2f}. Virtual cash remaining: Rs {portfolio['cash']:,.2f}."
            )
        else:
            answers.append(f"**Your paper trading position:** You currently hold no virtual shares of {ticker}.")
    ...
    if not answers:
        # Smart fallback: never leave the user with a dead-end. Give the risk summary as a default,
        # since "why" style questions almost always relate to risk/performance.
        label, _ = risk_label(context["risk_score"])
        answers.append(
            f"**General summary for {ticker}:** Risk score is {context['risk_score']}/100 ({label}), "
            f"with a total return of {context['total_return']:.2f}% over the selected period. "
            f"{len(context['volume_spikes'])} volume anomalies and {len(context['price_anomalies'])} price "
            f"anomalies were detected."
        )
        answers.append(
            "*(Your question didn't exactly match a specific topic, so here is a general summary instead. "
            "For more focused answers, try including words like: risk, volume, price, news, trend, "
            "fundamentals, or manipulation.)*"
        )

    return "\n\n".join(answers)


def generate_bull_bear_evidence(ticker, data, risk_score, volume_spikes, price_anomalies,
                                  pump_dump_df, divergence_df, total_return, ind_data):
    """Rule-based bull case vs bear case, built only from already-computed historical data."""
    bull_points = []
    bear_points = []

    if total_return > 0:
        bull_points.append(f"Total return over the selected period is positive at {total_return:.2f}%.")
    else:
        bear_points.append(f"Total return over the selected period is negative at {total_return:.2f}%.")

    if risk_score < 20:
        bull_points.append(f"Risk score is Low ({risk_score}/100), indicating few unusual price/volume patterns.")
    elif risk_score >= 50:
        bear_points.append(f"Risk score is High ({risk_score}/100), indicating a significant number of unusual patterns.")
    else:
        bull_points.append(f"Risk score is Moderate ({risk_score}/100) - not in the high-risk range.")

    if len(pump_dump_df) == 0:
        bull_points.append("No pump-and-dump style patterns were detected in the selected period.")
    else:
        bear_points.append(f"{len(pump_dump_df)} pump-and-dump style pattern window(s) were flagged.")

    if len(divergence_df) == 0:
        bull_points.append("No unusual volume-price divergence was detected.")
    else:
        bear_points.append(f"{len(divergence_df)} volume-price divergence flag(s) were detected.")

    latest_close = data["Close"].iloc[-1]
    if not ind_data.empty and pd.notna(ind_data["MA200"].iloc[-1]):
        ma200 = ind_data["MA200"].iloc[-1]
        if latest_close > ma200:
            bull_points.append(f"Current price (Rs {latest_close:.2f}) is above its 200-day moving average (Rs {ma200:.2f}).")
        else:
            bear_points.append(f"Current price (Rs {latest_close:.2f}) is below its 200-day moving average (Rs {ma200:.2f}).")

    if not ind_data.empty and pd.notna(ind_data["RSI"].iloc[-1]):
        rsi_val = ind_data["RSI"].iloc[-1]
        if rsi_val >= 70:
            bear_points.append(f"RSI is {rsi_val:.1f}, in overbought territory (70+), which can precede a pullback.")
        elif rsi_val <= 30:
            bear_points.append(f"RSI is {rsi_val:.1f}, in oversold territory (30 or below), which can indicate recent weakness.")
        else:
            bull_points.append(f"RSI is {rsi_val:.1f}, in a neutral range (not overbought or oversold).")

    if not bull_points:
        bull_points.append("No strong bullish signals were found in the historical data for this period.")
    if not bear_points:
        bear_points.append("No strong bearish signals were found in the historical data for this period.")

    return bull_points, bear_points


def generate_valuation_context(data):
    """Simple valuation heuristic: current price vs the stock's own historical average, since
    free data does not provide historical P/E series."""
    close = data["Close"]
    current_price = close.iloc[-1]
    avg_price = close.mean()
    median_price = close.median()
    pct_vs_avg = ((current_price - avg_price) / avg_price) * 100
    pct_vs_median = ((current_price - median_price) / median_price) * 100
    percentile = (close < current_price).mean() * 100
...
def generate_risk_trend(data, volume_multiplier, price_threshold_daily=7):
    """Splits the period into quarters and computes a simple anomaly-rate trend to show if
    unusual activity is increasing or decreasing over time."""
    if len(data) < 60:
        return None
    n = len(data)
    quarter_size = n // 4
    quarters = [
        data.iloc[0:quarter_size],
        data.iloc[quarter_size:2 * quarter_size],
        data.iloc[2 * quarter_size:3 * quarter_size],
        data.iloc[3 * quarter_size:n],
    ]
    rates = []
    for q in quarters:
        if len(q) == 0:
            rates.append(0)
            continue
        avg_vol = q["Volume"].rolling(window=min(10, len(q)), min_periods=1).mean()
        vol_flags = (q["Volume"] > avg_vol * volume_multiplier).sum()
        price_flags = (q["Close"].pct_change().abs() * 100 >= price_threshold_daily).sum()
        rate = (vol_flags + price_flags) / len(q) * 100
        rates.append(rate)
    ...
    if rates[-1] > rates[0] * 1.2:
        ...
        trend = "stable"
    return {"quarterly_rates": rates, "trend": trend}
...
# ---------------------------------------------------------------
    ...
    analyze_btn = st.session_state.get("analysis_shown", False)

    if analyze_btn:
        with st.spinner("Fetching data and running risk analysis..."):
            try:
                data = fetch_data(ticker_input, years, use_max=use_max_history)
                if use_max_history and not data.empty:
                    st.caption(f"Using maximum available history: {data.index[0].strftime('%Y-%m-%d')} to {data.index[-1].strftime('%Y-%m-%d')}.")
            except Exception as e:
        ...
        if data is not None and not data.empty:
            ...
            with tab1:
                ...
                st.markdown("#### Historical Valuation Context")
                st.caption(
                    "Free data does not provide a historical P/E series, so this compares the current price "
                    "to the stock's own historical average and median price over the selected period - a simple "
                    "proxy for whether the stock is trading high or low relative to its own history."
                )
                val_ctx = generate_valuation_context(data)
                v1, v2, v3 = st.columns(3)
                v1.metric("Current vs Historical Average", f"{val_ctx['pct_vs_avg']:+.1f}%")
                v2.metric("Current vs Historical Median", f"{val_ctx['pct_vs_median']:+.1f}%")
                v3.metric("Price Percentile (vs own history)", f"{val_ctx['percentile']:.0f}th percentile")

                st.markdown("#### Bull vs Bear Evidence")
                st.caption("Both sides are generated from the same historical data above - not a recommendation, just the evidence for each view.")
                ind_data_for_bull = calculate_technical_indicators(data)
                bull_points, bear_points = generate_bull_bear_evidence(
                    ticker_input, data, risk_score, volume_spikes, price_anomalies,
                    pump_dump_df, divergence_df, total_return, ind_data_for_bull
                )
                ...
                st.markdown("#### Risk Trend (Company vs Its Own History)")
                risk_trend_data = generate_risk_trend(data, volume_multiplier)
                if risk_trend_data:
                    trend_word = risk_trend_data["trend"]
                    trend_icon = "📈" if trend_word == "increasing" else ("📉" if trend_word == "decreasing" else "➡️")
                    st.markdown(f"{trend_icon} Unusual activity in this stock appears to be **{trend_word}** when comparing the earliest to the most recent quarter of the selected period.")
                    trend_fig = go.Figure(go.Bar(
                        x=["Q1 (oldest)", "Q2", "Q3", "Q4 (most recent)"],
                        y=risk_trend_data["quarterly_rates"]
                    ))
                    trend_fig.update_layout(height=280, yaxis_title="Anomaly rate (%)")
                    st.plotly_chart(trend_fig, use_container_width=True)
                else:
                    st.info("Not enough data in the selected period to compute a risk trend.")
            ...
            with tab2:
                ...
                show_bollinger = st.checkbox("Show Bollinger Bands (20-day, 2 std)", value=False)
                ...
                if show_bollinger:
                    price_fig.add_trace(go.Scatter(x=tf_ind_data.index, y=tf_ind_data["BB_Upper"], name="BB Upper", line=dict(color="#00ff88", width=1, dash="dot")))
                    price_fig.add_trace(go.Scatter(x=tf_ind_data.index, y=tf_ind_data["BB_Middle"], name="BB Middle", line=dict(color="#6b7280", width=1)))
                    price_fig.add_trace(go.Scatter(x=tf_ind_data.index, y=tf_ind_data["BB_Lower"], name="BB Lower", line=dict(color="#ff3366", width=1, dash="dot")))
                ...
                st.markdown("#### Average True Range (ATR, 14-day)")
                st.caption("Measures volatility - higher ATR means the stock has been moving more per day recently.")
                atr_fig = go.Figure()
                atr_fig.add_trace(go.Scatter(x=tf_ind_data.index, y=tf_ind_data["ATR"], name="ATR", line=dict(color="#ffcc00")))
                atr_fig.update_layout(height=220, yaxis_title="ATR (Rs)")
            ...
            with tab4:
                ...
                gemini_key_check = get_gemini_api_key()
                if gemini_key_check:
                    st.caption(
                        "Real AI mode is active (Google Gemini API connected, free tier). Ask in your own words "
                        "about this stock's risk, volume, price moves, news, fundamentals, or returns."
                    )
                else:
                    st.caption(
                        "Free rule-based mode (no AI API key configured yet). Ask about risk, volume, price moves, "
                        "news, fundamentals, or returns. Answers are pulled from the analysis already computed above - "
                        "not a prediction, not investment advice."
                    )
                user_question = st.text_input("Your question", placeholder="e.g. Why is the risk score high? What's my current P&L on this stock?", key="stock_qa_input")
                if st.button("Get Answer", key="stock_qa_btn"):
                    if user_question.strip():
                        portfolio_holdings = st.session_state.get("paper_holdings", {})
                        portfolio_log = pd.DataFrame(st.session_state.get("paper_log", []))
                        shares_held_qa = portfolio_holdings.get(ticker_input, 0)
                        avg_buy_qa = 0
                        if not portfolio_log.empty:
                            ticker_buys = portfolio_log[(portfolio_log["Ticker"] == ticker_input) & (portfolio_log["Action"] == "BUY")]
                            if not ticker_buys.empty:
                                avg_buy_qa = ticker_buys["Total"].sum() / ticker_buys["Qty"].sum()
                        latest_price_qa = float(data["Close"].iloc[-1])
                        unrealized_pnl_qa = (latest_price_qa - avg_buy_qa) * shares_held_qa if avg_buy_qa else 0
                        ...
                        qa_context = {
                            ...
                            "fundamentals": get_fundamentals(ticker_input),
                            "portfolio": {
                                "shares_held": shares_held_qa,
                                "avg_buy_price": avg_buy_qa,
                                "unrealized_pnl": unrealized_pnl_qa,
                                "cash": st.session_state.get("paper_cash", 0),
                            },
                        }
                        with st.spinner("Getting answer..."):
                            ai_answer = ask_real_ai(user_question, qa_context)
...
# ---------------------------------------------------------------
    ...
    st.markdown("---")
    st.markdown('<p class="ticker-tag">🕰️ PORTFOLIO TIME MACHINE</p>', unsafe_allow_html=True)
    st.caption(
        "Pick a past date, choose companies, and see what would have happened if you had invested "
        "a virtual amount on that date - using real historical prices up to today. This is a "
        "what-if simulation based on actual past data, not a prediction of the future."
    )

    tm_col1, tm_col2 = st.columns(2)
    with tm_col1:
        tm_start_date = st.date_input(
            "Investment start date",
            value=datetime.today() - timedelta(days=365),
            min_value=datetime.today() - timedelta(days=365 * 10),
            max_value=datetime.today() - timedelta(days=2),
            key="tm_start_date"
        )
    with tm_col2:
        tm_total_amount = st.number_input(
            "Total virtual amount to invest (Rs)", min_value=1000, value=100000, step=1000, key="tm_amount"
        )

    tm_companies = st.multiselect(
        "Companies to include (amount will be split equally)",
        list(COMPANIES.keys()), default=list(COMPANIES.keys())[:3], key="tm_companies"
    )

    if st.button("Run Time Machine", type="primary"):
        if not tm_companies:
            st.warning("Select at least one company.")
        else:
            per_stock_amount = tm_total_amount / len(tm_companies)
            tm_rows = []
            tm_fig = go.Figure()
            combined_value_series = None

            with st.spinner("Travelling back in time..."):
                for c_name in tm_companies:
                    c_ticker = COMPANIES[c_name]
                    try:
                        full_data = yf.download(c_ticker, start=tm_start_date, end=datetime.today(), progress=False)
                    ...
                    start_price = float(full_data["Close"].iloc[0])
                    end_price = float(full_data["Close"].iloc[-1])
                    shares_bought = per_stock_amount / start_price
                    end_value = shares_bought * end_price
                    pnl = end_value - per_stock_amount
                    pnl_pct = (pnl / per_stock_amount) * 100

                    tm_rows.append({
                        "Company": c_name, "Invested (Rs)": round(per_stock_amount, 2),
                        "Price on Start Date": round(start_price, 2), "Price Today": round(end_price, 2),
                        "Current Value (Rs)": round(end_value, 2), "P&L (Rs)": round(pnl, 2), "P&L %": round(pnl_pct, 2)
                    })

                    stock_value_series = (full_data["Close"] / start_price) * per_stock_amount
                    tm_fig.add_trace(go.Scatter(x=full_data.index, y=stock_value_series, name=c_name))

                    if combined_value_series is None:
                        combined_value_series = stock_value_series.reindex(full_data.index, method="ffill")
                    else:
                        combined_value_series = combined_value_series.add(
                            stock_value_series.reindex(combined_value_series.index, method="ffill"), fill_value=0
                        )

            if tm_rows:
                tm_df = pd.DataFrame(tm_rows)
                total_invested_tm = tm_df["Invested (Rs)"].sum()
                total_value_tm = tm_df["Current Value (Rs)"].sum()
                total_pnl_tm = total_value_tm - total_invested_tm
                total_pnl_pct_tm = (total_pnl_tm / total_invested_tm) * 100 if total_invested_tm else 0

                tmc1, tmc2, tmc3 = st.columns(3)
                tmc1.metric("Total Invested (on start date)", f"Rs {total_invested_tm:,.2f}")
                tmc2.metric("Value Today", f"Rs {total_value_tm:,.2f}")
                tmc3.metric("Total P&L", f"Rs {total_pnl_tm:,.2f}", f"{total_pnl_pct_tm:.2f}%")

                st.markdown("#### Growth Over Time (per company)")
                tm_fig.update_layout(xaxis_title="Date", yaxis_title="Value of Investment (Rs)", height=420)
                st.plotly_chart(tm_fig, use_container_width=True)

                st.markdown("#### Breakdown")
                st.dataframe(tm_df, use_container_width=True)

                tm_csv = tm_df.to_csv(index=False).encode("utf-8")
                st.download_button("Download Time Machine Report (CSV)", data=tm_csv,
                                    file_name="portfolio_time_machine.csv", mime="text/csv")
                st.caption(
                    "This simulation assumes a single lump-sum investment on the start date with no further "
                    "trades, and does not account for brokerage, taxes, or dividends."
                )
            else:
                st.error("Could not fetch historical data for the selected companies and date.")
